...
// packs it into a flow token, inserts a Path (RFC 3327) addressing this sequencer with
...
func (e *Engine) handleRegister(req *sip.Request, tx sip.ServerTransaction) {
	...
	token, err := mintFlowToken(flow)
	if err != nil {
		slog.Error("register: encode flow token", "callID", callIDValue(req), "err", err)
	...
	// transport (ws/wss) is carried inside the token, not on the Path. The Path
